...
def is_default_state_db_path(db_path) -> bool:
    """True when *db_path* is this process's call-time default ``state.db``."""
    default_path = _default_db_path()
    try:
        return Path(db_path).resolve() == default_path.resolve()
    except OSError:
        return Path(db_path) == default_path


def open_session_db_for_home(home, **kwargs) -> "SessionDB":
    """Open a profile's store without bypassing default-backend dispatch.

    An explicit ``db_path`` always names a SQLite file. The active profile home
    therefore uses ``SessionDB()`` so ``HERMES_STATE_STORE_DSN`` can dispatch
    it to PostgreSQL; non-active profile homes remain isolated SQLite stores.
    """
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    def _lock_holder_process_is_dead(self, holder: str) -> bool:
        """Backend seam for reclaiming structured lock holders before TTL."""
        ...
        def _do(conn):
            ...
            if row is not None:
                ...
                if (
                    ...
                    or self._lock_holder_process_is_dead(current_holder)
                ):
                    conn.execute(
                        "DELETE FROM compression_locks "
                        "WHERE session_id = ? AND holder = ?",
