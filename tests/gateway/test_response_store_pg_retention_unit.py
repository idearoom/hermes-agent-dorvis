...
class _FakeConnection:
    ...
    def execute(self, sql, params=None):
        ...
        if normalized.startswith(
            ...
            data, accessed_at, response_id, owner_id, owner_epoch, stale_before = params
            row = self.responses.get(response_id)
            matches = bool(
                row
                and row["owner_id"] == owner_id
                and row["owner_epoch"] == owner_epoch
                and row.get("owner_heartbeat_at") is not None
...
def test_pg_ownerless_legacy_put_expires(monkeypatch):
    ...
    try:
        ...
        assert store.recover_stale_owned("legacy", stale_before=time.time() + 1)
