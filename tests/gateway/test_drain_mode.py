...
class TestEcsTaskProtection:
    ...
    def test_cron_gate_observes_in_process_drain_before_marker_watcher(self):
        from gateway.run import _can_dispatch_in_process_cron

        runner = SimpleNamespace(
            _draining=False,
            _external_drain_active=False,
        )
        assert _can_dispatch_in_process_cron(runner)

        get_drain_mode().begin("sigterm")

        assert not _can_dispatch_in_process_cron(runner)
