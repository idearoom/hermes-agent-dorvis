...
def test_protected_tail_assistant_handoff_reaches_summarizer_boundary_context():
    """Completed tail work informs pending-state reconciliation without moving."""
    compressor = _compressor()
    messages = [
        {"role": "system", "content": "system prompt"},
        {"role": "user", "content": "prepare the release"},
        {"role": "assistant", "content": "started release preparation"},
        {"role": "user", "content": "collect the deployment inputs"},
        {"role": "assistant", "content": "deployment inputs collected"},
        {"role": "user", "content": "run the deployment"},
    ...
    boundary = captured.get("boundary_context", "")
    assert "deployment completed successfully" in boundary
    assert "now verify it" not in boundary
