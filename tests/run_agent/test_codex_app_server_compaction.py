...
class RaisingCodexSession(FakeCodexSession):
    def compact_thread(self):
        self.calls += 1
        raise RuntimeError("compaction transport failed")
...
def test_codex_app_server_dispatch_failure_closes_lifecycle_and_marks_usage():
    agent = DummyAgent(TurnResult())
    agent._codex_session = RaisingCodexSession(agent._codex_session.result)

    with pytest.raises(RuntimeError, match="compaction transport failed"):
        compress_context(
            agent,
    ...
    assert "primary:codex_compaction_dispatched_usage_unavailable" in usage[
        "warnings"
    ]
