...
class TestConcurrentToolExecution:
    ...
    def test_sequential_registry_tool_forwards_request_middleware_trace(
        ...
        mock_msg = _mock_assistant_msg(content="", tool_calls=[tool_call])
        def apply_request(_name, args, **kwargs):
            contexts["request"] = kwargs
        ...
        def run_execution(_name, args, callback, **kwargs):
            contexts["execution"] = kwargs
            return callback(args)

        def dispatch_pre(*_args, **kwargs):
            contexts["pre_hook"] = kwargs
            return None, None

        monkeypatch.setattr(
            "hermes_cli.middleware.apply_tool_request_middleware", apply_request
        )
        monkeypatch.setattr(
            "hermes_cli.middleware.run_tool_execution_middleware", run_execution
        )
        monkeypatch.setattr(
        ...
        assert observed[0]["request_metadata"] == agent._request_metadata
        assert contexts["request"]["request_metadata"] == agent._request_metadata
        assert contexts["execution"]["request_metadata"] == agent._request_metadata
        assert contexts["pre_hook"]["request_metadata"] == agent._request_metadata
