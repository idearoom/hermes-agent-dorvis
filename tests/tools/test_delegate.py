...
class TestDelegateTask(unittest.TestCase):
    ...
    def test_configured_shared_store_refuses_unpersisted_child_fallback(self):
        """A broken configured Postgres store must fail delegation loudly."""
        parent = _make_mock_parent(depth=0)
        parent._session_db = MagicMock(db_path=None)
...
class TestDelegateObservability(unittest.TestCase):
    ...
    def test_tool_trace_allows_null_error_field(self):
        """Structured output with error=null is a successful tool result."""
        parent = _make_mock_parent(depth=0)
        ...
        with patch("run_agent.AIAgent") as MockAgent:
            ...
            result = json.loads(delegate_task(goal="Test ok trace", parent_agent=parent))
            trace = result["results"][0]["tool_trace"]
            self.assertEqual(trace[0]["status"], "ok")
...
class TestDelegationProviderIntegration(unittest.TestCase):
    ...
    def test_request_metadata_reaches_child_agent(self, mock_creds, mock_cfg):
        ...
        with patch("run_agent.AIAgent") as MockAgent:
            ...
            self.assertEqual(
                metadata["delegation"]["parent_session_id"],
                "parent-session",
            )
            self.assertEqual(metadata["delegation"]["task_index"], 0)
            self.assertEqual(
                mock_child._request_metadata["delegation"]["child_session_id"],
                "child-session",
            )
