...
class TestDelegateObservability(unittest.TestCase):
    ...
    def test_tool_trace_allows_null_error_field(self):
        """Structured terminal JSON with error=null should remain ok."""
        parent = _make_mock_parent(depth=0)
        ...
        with patch("run_agent.AIAgent") as MockAgent:
            ...
            mock_child.run_conversation.return_value = {
                ...
                "messages": [
                    {"role": "assistant", "tool_calls": [
                        {"id": "tc_1", "function": {"name": "terminal", "arguments": '{"cmd": "printf ok"}'}}
                    ]},
                    {"role": "tool", "tool_call_id": "tc_1", "content": '{"output":"ok","exit_code":0,"error":null}'},
                ],
            }
            MockAgent.return_value = mock_child
