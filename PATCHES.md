...
| Pending skills REST review API (D15) | Source-only, not deployed | Expose the skills write-approval queue over authenticated list/diff/approve/reject API routes so hermes-web can own role-gated review. | `gateway/platforms/api_server.py`, `tests/gateway/test_api_server.py` | Low: additive API routes backed by existing approval commands | IdeaRoom Agentic Systems |
...
| Runtime Node 24 base image | Source-only, pending AWS promotion | Copy Node, npm, and corepack from the official Node 24 LTS slim image so the Hermes runtime and IdeaRoom CLI share the supported local/AWS Node major. | `Dockerfile`, `tests/tools/test_dockerfile_pid1_reaping.py` | Low: container build stage and runtime Node toolchain | IdeaRoom Agentic Systems |
