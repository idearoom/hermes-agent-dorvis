...
| Observer usage and cost disposition (AE-169) | Pending promotion | Send normalized token, cache, reasoning, cost, estimate-source, and estimate-status fields to observer hooks for every primary and attributable auxiliary provider dispatch. Unknown pricing remains explicitly unknown instead of being coerced to zero, while known zero cost remains representable. | `run_agent.py`, `agent/runtime_usage.py`, `tests/agent/test_observer_usage_cost.py`, `tests/agent/test_runtime_usage_attribution.py` | Low: additive observer-hook payload fields at the existing centralized usage seams | IdeaRoom Agentic Systems |
