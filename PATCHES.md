...
| Responses API Postgres store (D6a) | Promoted in fork history | Move the gateway Responses API store off EFS SQLite and into Postgres for concurrent web traffic. | `gateway/platforms/api_server.py`, `gateway/platforms/response_store_pg.py` | Medium: gateway response-store boundary | IdeaRoom Agentic Systems |
