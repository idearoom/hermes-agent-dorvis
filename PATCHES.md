...
| Hindsight retained-chat tagging | Promoted in fork history | Tag retained web chats with useful provenance for Hindsight recall and auditability, including PR review environment metadata/tags when traffic comes through Hermes Web. | `agent/turn_context.py`, `plugins/memory/hindsight/__init__.py` | Low: Hindsight plugin provider surface and turn-start metadata handoff | IdeaRoom Agentic Systems |
