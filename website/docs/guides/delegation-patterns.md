...
Subagents inherit the parent's enabled toolsets. `delegate_task` does not accept a model-facing `toolsets` parameter, so delegated work cannot grant itself capabilities that the parent does not have. Configure the parent's tools before starting the conversation when a delegated task needs web, terminal, file, or other access. Dorvis strips child-blocked tools such as `clarify`, `memory`, `send_message`, and `execute_code`; run mechanical scripted work from the parent instead.
