...
| `HERMES_DISABLE_ANYDOC` | Internal managed-runtime bridge for `security.allow_anydoc: false` in `config.yaml`. It disables the optional AnyDoc converter before import or lazy installation; native bounded DOCX, XLSX, and IPYNB extraction remains available. |
