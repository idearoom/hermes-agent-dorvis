...
| `HERMES_COMPRESSION_LOCK_WAIT_SECONDS` | Dorvis compatibility/manual override for how long an automatic compression-lock loser waits to adopt the winner's compacted transcript (default: `60`). Invalid values use the default, negative values become `0`, and the effective wait is always capped by the lock TTL so a dead holder cannot stall the turn beyond its reclaimable lease. |
...
| `HERMES_DRAIN_ON_SIGTERM` | Gateway drain mode (Dorvis fork, AE-117): when truthy, an **unplanned** SIGTERM begins a one-way drain — the API server refuses new run launches with `503 {"error": {"code": "gateway_draining"}}`, in-flight runs and their established SSE streams keep running, and the gateway self-exits (code 0) when active runs reach zero. Planned stops (`hermes gateway stop`, `--replace` takeover, Ctrl+C) are unaffected; a second SIGTERM while draining escalates to immediate shutdown. A drain can also be triggered explicitly via `POST /admin/drain` on the API server. Default: off. |
...
| `HERMES_TASK_PROTECTION_EXPIRES_MINUTES` | ECS task scale-in protection expiry per `PUT` from inside the task (default: `15`). Only meaningful when `ECS_AGENT_URI` is present (set by the ECS agent); outside ECS all protection calls are logged no-ops. |
| `HERMES_TASK_PROTECTION_RENEW_SECONDS` | ECS task scale-in protection renew cadence while active runs exist (default: `300`). |
