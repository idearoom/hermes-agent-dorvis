...
| `HERMES_DRAIN_ON_SIGTERM` | Gateway drain mode (Dorvis fork, AE-117): when truthy, an **unplanned** SIGTERM begins a one-way drain — API and relay launches are refused, already-admitted API/relay/cron work remains counted and protected, established SSE streams keep running, and the gateway self-exits (code 0) when all work reaches zero. API refusals use `503 {"error": {"code": "gateway_draining"}}`. Planned stops (`hermes gateway stop`, `--replace` takeover, Ctrl+C) are unaffected; a second SIGTERM while draining escalates to immediate shutdown. A drain can also be triggered explicitly via `POST /admin/drain` on the API server. Default: off. |
| `HERMES_DRAIN_CAP_SECONDS` | Gateway drain mode: hard cap on the drain window (default: `3600`). At the cap remaining runs are interrupted so their streams receive clean terminal events, then the gateway exits. |
| `HERMES_DRAIN_SETTLE_SECONDS` | Gateway drain mode: quiet period required at zero active runs before self-exit (default: `10`) so established SSE consumers can finish reading queued terminal events. |
| `HERMES_DRAIN_FORCE_GRACE_SECONDS` | Gateway drain mode: bounded wait after the drain-cap force-terminate for terminal events to flush before exiting (default: `30`). |
| `HERMES_TASK_PROTECTION_EXPIRES_MINUTES` | ECS task scale-in protection expiry per `PUT` from inside the task (default: `15`). API, relay, startup-resume, and cron admission wait for the first acknowledgement before agent execution; a failed API acknowledgement returns retryable `503 task_protection_unavailable`, while relay/cron work is not started. Only meaningful when `ECS_AGENT_URI` is present (set by the ECS agent); outside ECS all protection calls are logged no-ops. |
| `HERMES_TASK_PROTECTION_RENEW_SECONDS` | ECS task scale-in protection renewal cadence while active work exists (default: `300`). The effective cadence is capped at half of `HERMES_TASK_PROTECTION_EXPIRES_MINUTES`, and work-completion transitions wake release reconciliation immediately rather than waiting for the polling interval. |
| `HERMES_TASK_PROTECTION_HTTP_TIMEOUT_SECONDS` | Internal bridge for `gateway.task_protection_http_timeout_seconds` in `config.yaml` (default: `3`, bounded to `0.1`–`10`). Admission remains fail closed on timeout. |
| `HERMES_TASK_PROTECTION_FAILURE_BACKOFF_SECONDS` | Internal bridge for `gateway.task_protection_failure_backoff_seconds` in `config.yaml` (default: `2`). Concurrent/new admissions fail closed immediately during the backoff instead of each waiting on another blocking request. |
| `HERMES_RESPONSE_OWNER_HEARTBEAT_SECONDS` | Internal bridge for `gateway.response_owner_heartbeat_seconds` in `config.yaml` (default: `15`, minimum: `5`). |
| `HERMES_RESPONSE_OWNER_STALE_SECONDS` | Internal bridge for `gateway.response_owner_stale_seconds` in `config.yaml` (default: `300`; always at least four heartbeat intervals). Expired stored owners are recovered as terminal `incomplete` with reason `owner_lost`. |
...
| `HERMES_DISABLE_ANYDOC` | Internal managed-runtime bridge for `security.allow_anydoc: false` in `config.yaml`. A truthy operator-supplied value is a one-way runtime seal and cannot be weakened by config. It disables the optional AnyDoc converter before import or lazy installation; native bounded DOCX, XLSX, and IPYNB extraction remains available. |
