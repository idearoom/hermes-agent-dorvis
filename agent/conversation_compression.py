...
def _compression_diagnostics(agent: Any) -> dict[str, Any]:
    ...
    if not compressor:
        return {}
    # Target Hermes resolves context length lazily. Reading the public token
    # budget properties from an observability hook can otherwise turn event
    # emission into a synchronous provider probe. Prefer the cached backing
    # fields when the built-in compressor exposes them; legacy/plugin engines
    # without those fields retain their public-property behavior.
    threshold_tokens = (
        getattr(compressor, "_threshold_tokens", None)
        if hasattr(compressor, "_threshold_tokens")
        else getattr(compressor, "threshold_tokens", None)
    )
    tail_token_budget = (
        getattr(compressor, "_tail_token_budget", None)
        if hasattr(compressor, "_tail_token_budget")
        else getattr(compressor, "tail_token_budget", None)
    )
    return {
            ...
            def _finish_lock_contention_attempt(
                *,
                ...
                _complete_compaction_lifecycle(
                    force_terminal=commit_status != "committed"
                )
