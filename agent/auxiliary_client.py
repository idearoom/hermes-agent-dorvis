...
def _relay_sync_completion(
    ...
    *,
    ...
    if route is None:
        ...
        dispatch = lambda: _run_protected_sync_provider_call(callback, kwargs)
    else:
        provider_name, fallback_model, metadata = route
        from agent import relay_llm

        dispatch = lambda: relay_llm.execute_current(
            kwargs,
    ...
    return _tracked_auxiliary_create(
        client,
        kwargs,
...
async def _relay_async_completion(
    ...
    *,
    ...
    context = _RELAY_AUX_CALL_CONTEXT.get() or {}
    return await _tracked_auxiliary_create_async(
        client,
        kwargs,
        str(context.get("task") or "unknown"),
        provider=provider_name,
...
def _relay_sync_stream(
    ...
    *,
    ...
    if route is None:
        provider_name = str(provider or "auxiliary")
        dispatch = lambda: client.chat.completions.create(**kwargs)
    else:
        provider_name, fallback_model, metadata = route
        from agent import relay_llm

        dispatch = lambda: relay_llm.stream_current(
            kwargs,
...
def _tracked_auxiliary_create(
    ...
    *,
    ...
    return track_auxiliary_dispatch(
        dispatch or (lambda: client.chat.completions.create(**kwargs)),
        task=task,
        provider=provider,
        model=str(kwargs.get("model") or ""),
        base_url=str(getattr(client, "base_url", "") or ""),
