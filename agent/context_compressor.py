...
class _CompressionQualityGate:
    """Deterministic acceptance policy for a proposed compression rewrite.

    The compressor owns orchestration and state rollback; this object owns the
    quality decision. Keeping that policy self-contained and side-effect-free
    makes the fork seam independently testable while preserving the historical
    ``ContextCompressor`` diagnostic attributes used by the gateway.
    """

    __slots__ = ("min_savings_pct", "max_post_threshold_ratio")

    def __init__(
        self,
        *,
        min_savings_pct: float,
        max_post_threshold_ratio: float,
    ) -> None:
        self.min_savings_pct = max(0.0, min(float(min_savings_pct), 100.0))
        self.max_post_threshold_ratio = max(
            0.10,
            min(float(max_post_threshold_ratio), 1.0),
        )

    @staticmethod
    def extract_state_ledger_fields(summary: str) -> Dict[str, bool]:
        fields = {key: False for key in STATE_LEDGER_KEYS}
        in_ledger = False
        for raw_line in str(summary or "").splitlines():
            line = raw_line.strip()
            if line.startswith("## "):
                in_ledger = line.casefold() == "## state ledger"
                continue
    ...
    def _latest_user_text(messages: List[Dict[str, Any]]) -> str:
        for message in reversed(messages):
            if message.get("role") == "user":
                return _content_text_for_contains(message.get("content")).strip()
        return ""
    ...
    def evaluate(
        ...
        *,
        ...
        reasons: List[str] = []
        ledger_fields = self.extract_state_ledger_fields(summary)
        ...
        if pre_tokens and post_tokens:
            ...
            elif post_tokens > int(
                threshold_tokens * self.max_post_threshold_ratio
            ):
        ...
        if latest_user:
            ...
            # Full-text match catches ordinary messages. The prefix gives
            # multimodal and very long prompts a stable deterministic anchor.
...
class ContextCompressor(ContextEngine):
    ...
    def __init__(
        ...
        self._quality_gate = _CompressionQualityGate(
            min_savings_pct=quality_gate_min_savings_pct,
            max_post_threshold_ratio=quality_gate_max_post_threshold_ratio,
        )
        # Keep these names stable for diagnostics and config introspection.
        self.quality_gate_min_savings_pct = self._quality_gate.min_savings_pct
        self.quality_gate_max_post_threshold_ratio = (
            self._quality_gate.max_post_threshold_ratio
        )
...
This compaction should PRIORITISE preserving all information related to the focus topic above. For content related to "{focus_topic}", include full detail — exact values, file paths, command outputs, error messages, and decisions. For content NOT related to the focus topic, summarise more aggressively (brief one-liners or omit if truly irrelevant). The focus topic sections should receive roughly 60-70% of the summary token budget. Even for the focus topic, NEVER preserve API keys, tokens, passwords, or credentials — use [REDACTED]."""
    ...
    def _extract_state_ledger_fields(cls, summary: str) -> Dict[str, bool]:
        """Compatibility wrapper around the side-effect-free gate policy."""
        return _CompressionQualityGate.extract_state_ledger_fields(
            cls._strip_summary_prefix(summary or "")
        )
    ...
    def _evaluate_compression_quality(
        ...
        *,
        ...
        """Evaluate a candidate while preserving the historical method seam."""
        policy = getattr(self, "_quality_gate", None)
        if policy is None:
            # Several focused upstream tests construct the compressor through
            # ``object.__new__``. Preserve that test seam without spreading
            # policy defaults through the orchestration path.
            policy = _CompressionQualityGate(
                min_savings_pct=getattr(
                    self,
                    "quality_gate_min_savings_pct",
                    10.0,
                ),
                max_post_threshold_ratio=getattr(
                    self,
                    "quality_gate_max_post_threshold_ratio",
                    0.80,
                ),
            )
    ...
    def compress(
        ...
        # getattr: upstream tests construct ContextCompressor via __new__
        # (bypassing __init__), so the Dorvis quality-gate flag may be unset.
        if getattr(self, "quality_gate_enabled", False):
            ...
            telemetry["quality_gate_passed"] = passed
            telemetry["quality_gate_reasons"] = list(reasons)
            if not passed:
                self._last_compress_aborted = True
                self._last_summary_error = (
                    "compression quality gate failed: " + ", ".join(reasons)
                )
                telemetry["failure_class"] = "compression_quality_gate_failed"
                self._previous_summary = _previous_summary_before_scan
                self._summary_has_user_turn = _summary_has_user_turn_before_scan
                if not self.quiet_mode:
                    logger.warning(
            ...
            telemetry["quality_gate_passed"] = True
            telemetry["quality_gate_reasons"] = []
