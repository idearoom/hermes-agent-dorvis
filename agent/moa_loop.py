...
def _redact_trace_accounting(acct: Any) -> Any:
    ...
    return _RefAccounting(
        ...
        acct.cost_status,
        acct.cost_source,
        usage_complete=acct.usage_complete,
        usage_warnings=acct.usage_warnings,
...
def _run_reference(
    ...
    *,
    ...
    try:
        ...
        # ``call_llm`` can dispatch multiple physical provider attempts before
        # returning one accepted response. Observe every attempt so an
        # abandoned retry cannot disappear from headless usage completeness,
        # while exact billable responses remain identity-deduplicated.
                ...
                if future.cancel():
                    ...
                    results[idx] = (
                        ...
                        _RefAccounting(
                            CanonicalUsage(request_count=0),
                            usage_complete=False,
                            usage_warnings=("reference_cancelled_before_dispatch",),
                        ),
                    )
                elif future.done():
                    ...
                    results[idx] = (
                        ...
                        _RefAccounting(
                            CanonicalUsage(request_count=0),
                            usage_complete=False,
                            usage_warnings=("reference_interrupted_in_flight",),
                        ),
                    )
...
class MoAChatCompletions:
    ...
    def create(self, **api_kwargs: Any) -> Any:
        ...
        if _refs_from_cache:
            ...
            # Likewise preserve an unconsumed trace from the cache MISS. An
            # aggregator failure can re-enter create() with identical state
            # before the conversation loop flushes the trace; clearing it on
            # that retry would silently lose the reference fan-out evidence.
        else:
