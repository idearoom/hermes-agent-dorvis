...
class SessionStore:
    ...
    Uses SessionDB for session metadata and message transcripts. Local SQLite
    may fall back to legacy JSONL, but an explicitly configured Postgres store
    ...
    def _open_session_db_for_active_scope(self):
        ...
        postgres_required = bool(
            os.getenv("HERMES_STATE_STORE_DSN", "").strip()
        )
        ...
        def _open():
            try:
                ...
                if postgres_required:
                    raise RuntimeError(
                        "HERMES_SESSION_STORE_POSTGRES_REQUIRED"
                    ) from e
        ...
        return self._db_handle_cache.get(
            path,
            _open,
            raise_on_error=postgres_required,
            non_cacheable=lambda exc: (
                isinstance(exc, RuntimeError)
                and (
                    "live-system guard" in str(exc)
                    or "HERMES_SESSION_STORE_POSTGRES_REQUIRED" in str(exc)
                )
            ),
        )
