    ...
    "parameters": {
        ...
        "properties": {
            ...
            "session": {
                "type": "string",
                "description": (
                    "Optional friendly name for a task-scoped isolated browser "
                    "session. Each distinct name gets its own harness daemon (and "
                    "on provider backends its own browser), so concurrent tasks do "
                    "not share tabs. Reuse a name across related calls; omit to "
                    "reuse the task default. At most "
                    f"{_MAX_DAEMONS_PER_TASK} sessions may be active for one task."
                ),
            },
            "timeout_s": {
                "type": "integer",
