...
def _build_child_request_metadata(
    ...
    *,
    ...
    """Copy parent web/request metadata and add subagent lineage.

    The observability plugin keys Langfuse user/session/chat attribution from
    request_metadata.  Delegated children are fresh AIAgent sessions, so they
    must explicitly inherit that metadata to remain queryable with the parent
    web chat.  The child_session_id is filled in after AIAgent construction.
    """
