...
def _build_child_agent(
    ...
    if parent_session_db is not None:
        try:
            ...
            # hide a broken Postgres/schema deployment behind a successful
            # delegation response.
            if os.environ.get("HERMES_STATE_STORE_DSN", "").strip():
                logger.error(
                    "HERMES_STATE_STORE_REQUIRED: failed to open dedicated "
                    "SessionDB for delegated child",
                    exc_info=True,
                )
                raise
            logger.debug(
...
def _rollup_child_usage(
    parent_agent,
    entry: Dict[str, Any],
    child_usage: Optional[Dict[str, Any]],
) -> None:
    """Fold one child's complete usage snapshot into its parent once."""
    if parent_agent is None:
        return
    ...
    try:
        ...
        child_input = child_usage.get("input_tokens")
        child_output = child_usage.get("output_tokens")
        valid_input = (
            isinstance(child_input, int)
            and not isinstance(child_input, bool)
            and child_input >= 0
        )
        valid_output = (
            isinstance(child_output, int)
            and not isinstance(child_output, bool)
            and child_output >= 0
        )
        public_total = (
        ...
        completeness = child_usage.get("completeness", "partial")
        if entry.get("status") in {"timeout", "interrupted"}:
            completeness = "partial"
        entry["tokens"] = {
            "input": child_input if valid_input else 0,
            "output": child_output if valid_output else 0,
            "total": public_total,
...
def _finalize_child_results(
    ...
    with _parent_finalization_lock(parent_agent):
        ...
        for entry in results:
            ...
            _rollup_child_usage(parent_agent, entry, child_usage)
            try:
                if child_cost:
            ...
            try:
                ...
                invoke_hook(
                    ...
                    child_session_id=(
                        entry.get("child_session_id")
                        or getattr(child, "session_id", None)
                    ),
                    ...
                    request_metadata=(
                        getattr(parent_agent, "_request_metadata", None) or {}
                    ),
                )
            except Exception:
